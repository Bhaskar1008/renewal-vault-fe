        ...
        <div class="testimonials-form pa-3">
            ...
            <v-layout align-center d-flex>
             ...
             <v-layout row wrap align-center style="cursor:pointer" class="mb-4">
                            <input ref="planPictures" id="imgdragger" name="media_upload" type="file" accept="image/x-png,image/jpeg" class="dragFile" @change="handleFileUpload"/>
                            ...
                            <!-- <v-flex xs2 @click="validateFiles()">
        ...
        <v-layout align-center justify-space-between row fill-height class="mb-5">
                            <div>
                                <v-btn color="#01B4BB" class="white--text" @click="save">{{formFields.HTTP === 'PUT' ? 'Save' : 'Submit'}}</v-btn>
                                 <v-btn  color="primary" class="white--text thebtn" @click="resetFields">Close</v-btn>
                            </div>
                            <!-- <v-btn flat style="background-color: #F5F5F5" @click="closeit">Close</v-btn> -->
        </v-layout>
...
<!--                       
            ...
            <v-data-table
                ...
                <template slot="items" slot-scope="props">
                    ...
                    <td class="text-xs-left">{{ props.item.presentation_id }}</td>
...
export default {
    ...
    created() {
        ...
        // this.fetchAll();
        this.$store.commit('SET_PAGE_TITLE','Presentation');
    ...
    data() {
        return {
            ...
            indexofedit:null,
            planImages: {
                text: "Drag Presentation Images here",
                file: '',
                respFileName: '',
                respFileType: ''
            },
    ...
    methods: {
     ...
     handleMediaActivationDeactivation(data, index){
                 this.formFields.mediaFiles[index].active = data;
     }, 
    ...
    dis(){
        ...
        fetchAll() {
            ...
            const FETCH_TESTIMONIALS = 'fetch_sales_pitch?type=presentation&filter=0';

           this.GET(FETCH_TESTIMONIALS, res => {

            // axios
            // .get(FETCH_TESTIMONIALS)
            // .then(res => {
                self.testimonials = [];
        ...
        setValuesOnForm(res) {
            ...
            for(let i=0; i<res.filePath.length; i++){
                    this.formFields.mediaFiles.push({
                            active: res.filePath[i].active,
                            fileName: res.filePath[i].fileName,
                            fileType: res.filePath[i].fileType,
                            location:  res.filePath[i].location,
                    });
            }
        ...
        resetFields() {
         this.planImages.text = "Drag Presentation Images here"

            let props = null;
            for(props in _dup_formFields) {
                this.formFields[props] = _dup_formFields[props]
                //Clears the tabel's and images
        ...
        handleFileUpload(event, indicator, index) {
             ...
             */
            console.log("Are we posting from Upload button?")
        ...
        validateFiles(id, file) {
            ...
            sendFile = file;
            console.log("Senf Files->", sendFile);
            if (id == 5) {
        ...
        uploadFile(sendFile) {
            ...
            const UPLOAD_API = this.API_Service_admin()+'v2/uploadFile'
            axios
                ...
                // if (_this.e1 == 2) {
                    ...
                    if (res.data.errMsg.fileType == 'mp4') {
                        ...
                        if(  this.formFields.editFile == true){
                              ...
                              _this.formFields.mediaFiles.push(res.data.errMsg);
                               console.log("RESET DRAG IMG->", document.getElementById("imgdragger").value);
                        }
        ...
        save() {
            ...
            if (this.formFields.title == null || this.formFields.content == "" || this.formFields.content == null || this.formFields.title == undefined || this.formFields.title == "" || this.channeldatafilter === "" || this.formFields.mediaFiles.length == 0) {
                this.showToast('Mandatory fields are not found' , this.TOST().WARNING);
                this.e1 = 2;
                return;
            }

            console.log("Media FILES->", this.formFields.mediaFiles);
         this.presentationimages=[];
        for(let i=0; i< this.formFields.mediaFiles.length; i++){

                this.presentationimages.push({
                    active: this.formFields.mediaFiles[i].active,
                    fileName: this.formFields.mediaFiles[i].fileName,
                    fileType: this.formFields.mediaFiles[i].fileType,
                    location:  this.formFields.mediaFiles[i].location,
                })
        }

         console.log("IMAGES ARRAY->", this.presentationimages);
            // this.presentationimages.push()
            ...
            var formData = {
                ...
                filePath: this.presentationimages,
        ...
        doPost(formData) {
            const INSERT_TESTIMONIALS = 'new_sales_pitch?type=presentation';
        ...
        doPut(formData) {
            ...
            const UPDATE_TESTIMONIALS = 'update_sales_pitch?type=presentation';
