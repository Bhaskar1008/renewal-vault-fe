...
<template>
  <div id="log_wrapper">
    <v-card>
      <v-layout row wrap>
        <!-- <v-card flat style="background-color:#FAFAFA; width:100%;"><v-layout wrap justify-space-between class="pt-4 ml-3 mr-3 " color="#FAFAFA"><v-flex xs10 sm6 md2 class=" mt-1 mr-4"><v-select v-model="channeldatafilter" :items="channeldataall" item-text="text" item-value="value" label="Channel" @change="channelfilters"></v-select></v-flex><v-flex></v-flex><v-menu transition="slide-y-transition"><v-btn depressed medium slot="activator" color="secondary" @click="openwindow" class="white--text thebtn">Bulk Upload
                                <v-icon dark right>publish</v-icon></v-btn></v-menu><v-flex xs10 sm6 md2 class="ml-2"><p>{{selec_emp_code}}</p><v-text-field @keyup.enter="startsearch" v-model="selec_emp_code" label="Search" append-icon="search" height="30px"></v-text-field></v-flex></v-layout></v-card> -->
        <div>
          <hr>
          </div>
          <!-- ml-3 mr-3 mt-3 -->
          <v-flex class="" style="flex-flow:column wrap;width:100%;">
                    ...
                    <v-card-text>
                      <v-flex xs12 sm6 md6>
                        ...
                        <v-layout align-center row wrap>
                          ...
                          <!-- <v-flex xs6 sm3><p class="caption font-italic font-weight-medium" style="color:#9B9B9B">Supports mp4 format. Approx time to upload 2mb file - 30 min</p></v-flex> -->
                        </v-layout>
                      </v-flex>
            ...
            <v-card flat class="pa-4">
              <v-flex justify-center style="display: flex;">
                <v-flex justify-center md12 sm6 >
                  <h3 class="text-xs-center mb-3">KPI Details</h3>
                  <v-data-table v-model="selected" :headers="headers" :items="kpidata" hide-actions  item-key="_id" class="elevation-1">
                    <template slot="items" slot-scope="props">
                      <td class="">{{props.item.uploadeddate}}</td>
                      <td class="">{{props.item.channelname}}</td>
                      <td class="">{{props.item.uploadedby | to-capitlaize }}</td>
                      <td class="">{{props.item.period | to-capitlaize }}</td>
                      <td class="align-center" >{{props.item.totalrecords}} 
                        <v-btn v-if="props.item.totalrecords > 0" flat icon @click="Downloadfile('all',props.item._id)">
                          <v-icon color="primary">get_app</v-icon>
                        </v-btn>
                      </td>
                      <td class="align-center" >{{props.item.successfullyinserted}}  
                        <v-btn v-if="props.item.successfullyinserted > 0" flat icon @click="Downloadfile('success',props.item._id)">
                          <v-icon color="primary">get_app</v-icon>
                        </v-btn>
                      </td>
                      <td class="align-center">{{props.item.failinserted}} 
                        <v-btn v-if="props.item.failinserted > 0" flat icon @click="Downloadfile('failed',props.item._id)">
                          <v-icon color="primary">get_app</v-icon>
                        </v-btn>
                      </td>
                    </template>
                    ...
                    <v-stepper-content step="1">
                      <v-layout row class="pa-2">
                        <div style="display:flex; flex-direction:column;justify-content: center;">
                          <p style="font-size:20px; font-weight:500;" >Please Select a Month</p>
                          <p >Your Selected month for Uploading the KPI sheet is: 
                            <span class="selectedmon">{{moment(datemonth).format('MMMM')}} | {{datemonth}}</span>
                          </p>
                        </div>
                        <v-flex class="picit">
                          <v-date-picker
                      ...
                      <v-layout>
                        ...
                        <v-layout mt-3 justify-space-between>
                          <a href="https://image-upload-bucket-2019.s3.ap-south-1.amazonaws.com/40fde0c3cdf059441c40d477e2be76b6d74eec27.xlsx" download>
                        ...
                        <v-card-actions style="justify-content:flex-end;">
                          <v-layout mt-5 justify-space-between>
                            <v-btn color="primary" @click="steps =1 ">
                              <v-icon class="mr-2">arrow_back</v-icon>Previous
                            </v-btn>
                    ...
                    <v-flex v-if="showAlertext == true">
                      <v-data-table ref="vTable" v-model="popTable" :headers="popHeader" :items="agentObj" hide-actions :pagination.sync="pagination" item-key="_id" class="elevation-1">
                        <template slot="items" slot-scope="props">
                          ...
                          <!-- <v-radio-group v-model="radios" row><v-radio :value="props.item._id" @change="agentTable(props.item._id,$event)"></v-radio></v-radio-group> -->
                          <!-- </td> -->
...
export default {
    ...
    created(){
        // http://172.16.60.38:5010/secure/user/get_kpi_logs
        ...
        this.$store.commit('SET_PAGE_TITLE', 'KPI Logs');
    ...
    data() {
        return {
            ...
            headers: [
                {
                    text:"Date and Time of Upload",
                    value:"DateandTimeofUpload"
                },
                {
                    text:"Channel Name",
                    value:"channelname"
    ...
    methods: {
        ...
        Downloadfile(event , id){
            console.log('Download function ',event,id)
            // http://172.16.58.99:5010/secure/admin/exportLog/5b3b4cc28fa96d39870443e3?filter=success&logId=5e41014d9c251318dc98d61a
            let userid = this.$store.state.agentDetails._id
            let self = this;
            this.GET('exportLog/'+userid+'?filter='+event+'&logId='+id, (res, error) => {
                if (error) {
                ...
                if (res.data.errCode === -1) {
                    self.showToast('File exported successfully', self.TOST().SUCCESS)
                    console.log(res);
                    console.log("Export API",res.data.errMsg) 
                    let anchorElement = document.createElement('a');
                    anchorElement.style.display = 'none';
                    anchorElement.setAttribute('href', res.data.errMsg);
                    anchorElement.setAttribute('download', 'Hello.xlsx');
                    document.getElementById('log_wrapper').appendChild(anchorElement);
                    anchorElement.click();
                    document.getElementById('log_wrapper').removeChild(anchorElement);
                }
                // }
            })
        },

        fetchkpilogsdata(skip, channel){
            ...
            if(channel == 'all'){
                ...
                build_api = 'get_kpi_logs/'+userid+'?skip='+skip 

            }else{
                build_api = 'get_kpi_logs/'+userid+'?skip='+skip+'&channel='+channel
            }

            console.log("BUILD_API is =>",  build_api);

            this.GET(build_api ,res => {
                // console.log('KPI LOGS:::::',res.data.errMsg[0])
                let logsresponse = res.data.errMsg[0];
                let totalCount = res.data.errMsg[1].count
                ...
                for(let i=0;i<logsresponse.length;i++){
                    // moment().format('LT');
                    db_struct.uploadeddate = moment(logsresponse[i].uploadedDate).format('lll')
                    db_struct.channelname = logsresponse[i].channel.channelName == null ?  '-' : logsresponse[i].channel.channelName == undefined ? "-" : logsresponse[i].channel.channelName
                    db_struct.uploadedby = logsresponse[i].uploadedBy.first_name+' '+logsresponse[i].uploadedBy.last_name
                    if(logsresponse[i].qauter === '' || logsresponse[i].qauter === null || logsresponse[i].qauter === undefined ){
                        db_struct.period = logsresponse[i].month+' '+logsresponse[i].year
                    }else{
                        console.log('')
                        if(logsresponse[i].qauter.localeCompare('q1')=== 0){
                        db_struct.period = 'Quarter 1 '+ logsresponse[i].year
                        }else if(logsresponse[i].qauter.localeCompare('q2')=== 0){
                        db_struct.period = 'Quarter 2 '+ logsresponse[i].year
                        }else if(logsresponse[i].qauter.localeCompare('q3')=== 0){
                        db_struct.period = 'Quarter 3 '+ logsresponse[i].year
                        }else if(logsresponse[i].qauter.localeCompare('q4')=== 0){
                        db_struct.period = 'Quarter 4 ' + logsresponse[i].year
                        }else{
                            console.log('ELSEEEEEEE')
                        }
                        // db_struct.period = 'Quater '+logsresponse[i].year
                    }
                    db_struct.totalrecords = logsresponse[i].totalRecords
                    db_struct.successfullyinserted = logsresponse[i].success
                    db_struct.failinserted = logsresponse[i].failed
                    db_struct._id = logsresponse[i]._id
                    this.kpidata.push(db_struct)
                    db_struct = {}
                }
                console.log('KPI FILTER DATA',this.kpidata);
            })
        },
        ...
        channelfilters(e) {
            console.log('dropdown data', e)
            this.page = 1
            this.channeldatafilter = e
            console.log("v-mode is ->", this.channeldatafilter);
            this.fetchkpilogsdata(0,  this.channeldatafilter);
            // return
        ...
        next(e) {
            ...
            this.fetchkpilogsdata(this.skip,this.channeldatafilter)
        },
        ...
        get_kpi_dash_data(incomeingdata ,skip){
            ...
            // }
            get_kpi_data = 'get_kpi_logs'
              
            this.GETUSER(get_kpi_data, res => {
                console.log('Response Data is ->', res.data.errMsg);
                this.dummydata = []
                var resdata = [];
                resdata = res.data.errMsg;
                // var pageLen = res.data.errMsg[1]
                ...
                for(let i=0; i<reslen; i++){
                    this.kpitable = {
                        date: resdata[i].uploadedDate,
                        month: resdata[i].month,
                        year:resdata[i].year,
                        uploadedBy:resdata[i].uploadedBy.email,
                    }
