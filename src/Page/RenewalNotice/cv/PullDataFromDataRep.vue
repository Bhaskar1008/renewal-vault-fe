  ...
  <v-app style="background-color: #b5c1c433">
    <div style="margin-left: 35px" class="mt-4">
      <v-text class="body-2 font-weight-bold" style="color: #1e83c2"
        >Upload Data From Data Rep</v-text
      >
    </div>
    <v-card class="ma-3"
    style="cursor: auto;"
        ...
        <v-layout>
          <v-flex xs1 class="ml-3">
            <p
              ...
              @click="
                downloadData(
                  'https://cv-ccm-releated-files.s3.ap-south-1.amazonaws.com/sample-pull-policies.xlsx'
                )
              "
            >
              <a>Sample File</a>
  ...
  methods: {
    ...
    excelUploadHandler() {
      ...
      if (self.selectFileData && self.policyLOB.NUM_IL_PRODUCT_NAME) {
        ...
        apiService("post", cvApiPath.pullExcelData, formData)
          .then((res) => {
    ...
    getExcelMovementHistory(status = "ALL") {
      ...
      let user = JSON.parse(localStorage.loginResp);
      self.dataLoader = true;
      apiService("post", `${cvApiPath.getExcelpoliciesPulledDataHistory}`, {
