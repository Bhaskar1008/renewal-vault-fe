        ...
        <v-layout row wrap>
            ...
            <v-flex class="ml-3 mr-3 mt-3" style="flex-flow:column wrap;width:100%;">
                ...
                <v-flex justify-center md12 ml-3 mt-3 mr-5 mb-4>
                    <h3 class="text-xs-center mb-3">Producer KPI Details</h3>
                    <v-data-table v-model="selected" :headers="headers" :items="dummydata" hide-actions item-key="_id" class="elevation-1">
                        <template slot="items" slot-scope="props">
                            ...
                            <!-- </td> -->
                            <td class="">{{props.item.prod_name}}</td>
                            <td class="">{{props.item.prod_id}}</td>
                            <td class="">{{props.item.target}}</td>
                            <!-- <td class="">{{props.item.prod_business}} <v-icon small >edit</v-icon></td> -->
                            <td>
                                <v-edit-dialog :return-value.sync="props.item.prod_business" large lazy @save="saveDataProd(props.item)"> 
                                    {{ props.item.prod_business }} <div style="margin-left:10px"><v-icon small >edit</v-icon></div>
                                    <template slot="input">
                                        <v-text-field v-model="props.item.prod_business" label="Edit" single-line counter autofocus></v-text-field>
                                    </template>
                                </v-edit-dialog>
                            </td>
                            ...
                            <!-- </td>-->
                            <td class="">{{props.item.current_club}}</td>
                            <td class="">{{props.item.next_club}}</td>
                            <td class="">{{props.item.balance}}</td>
                            <!-- <td class="">
                                <v-icon small class="" @click="editKpiProd(props.item)">edit</v-icon>
                            </td> -->
                        </template>
                    </v-data-table>
                </v-flex>
                            ...
                            <v-stepper-items>
                                <v-stepper-content step="1">
                                  ...
                                  <!-- <v-layout row class="pa-2">
                                        <v-flex style="display:flex; flex-direction:column;justify-content: center;"> <p style="font-size:20px; font-weight:500;" >Please Select a Month</p>
                                    ...
                                    <v-layout mt-3 justify-space-between>
                                        <a href="https://image-upload-bucket-2019.s3.ap-south-1.amazonaws.com/0d1ef0aae41ed3a5a863229d8d7980a2b1418584.xlsx" download>
...
export default {
    ...
    created(){
        ...
        this.get_producer_kpi_dash_data('all',0);
        ...
        this.$store.commit('SET_PAGE_TITLE', 'Producer KPI');
    ...
    methods: {
        ...
        saveDataProd (prod) {
            console.log('Dialog closed_prod',prod)
            let logUserData = this.$store.getters.getAgentDetails
            let prodData = {
                producerId: prod._id_prod,
                bussinessAmount:prod.prod_business,
                userId:logUserData._id,
            }
            this.PUT('updateBussinessAmount', prodData, res => {
                console.log('PRODD ind',res.data.errMsg)

                this.producer_table ={
                    prod_name:this.checkValidity(prod.prod_name),
                    prod_id: this.checkValidity(prod.prod_id),
                    prod_business: this.checkValidity(res.data.errMsg.bussiness),
                    target: this.checkValidity(prod.target),
                    balance: this.checkValidity(prod.balance),
                    current_club: this.checkValidity(prod.current_club),
                    next_club: this.checkValidity(prod.next_club),
                    _id_prod: this.checkValidity(prod._id_prod),
                },
                 this.dummydata.push(this.producer_table)
            })
            // console.log('PRODD ind',ind)
        ...
        channelfilters(e) {
            ...
            this.get_producer_kpi_dash_data(this.channeldatafilter,this.skip)
        },
        // n99
        next(e) {
            let skipdata = e - 1
        ...
        goNext() {
            // alert('firing prodcuer post api');
          ...
          if( this.bulkData.length == 0){
                this.showToast('Producer KPI Sheet is empty', this.TOST().ERROR);
                return
            }
            ...
            let self = this;
            var producer_kpi = 'updatePartnerInBulk/'+this.getAgentInfo()._id;
             console.log("FormData before sending is", formData);
            this.PUT(producer_kpi, formData, function(res, error) {
                        console.log("res is", res)
                    self.showLoader('Uploading', false);

                    if (res.data.errCode == -1) {
                        console.log("producer kpi link",res.data.errMsg);
        ...
        get_producer_kpi_dash_data(e , skip){
            ...
            if(e == 'all'){
                ...
                // fetch_producers/5b3b4cc28fa96d39870443e3?skip=0
                get_kpi_data ='fetch_producers/'+this.getAgentInfo()._id+'?skip='+skip
            }else{
                console.log('CHANNEL WISE HIT');
                // get_kpi_data ='fetch_producers/'+this.getAgentInfo()._id+'&channel='+e+'?emp_code='+this.selec_emp_code
                get_kpi_data ='fetch_producers/'+this.getAgentInfo()._id+'?skip='+skip+'&channel='+e

            }

            this.GET(get_kpi_data, (res,error)=>{
            // axios.get(get_kpi_data).then(res => {
                ...
                if(res.data.errCode === -1){
                    ...
                    for(let i=0; i < reslen; i++){
                        ...
                        this.producer_table ={
                                prod_name:this.checkValidity(resdata[i].producerName),
                                prod_id: this.checkValidity(resdata[i].producerID),
                                prod_business: this.checkValidity(resdata[i].bussiness),
                                target: this.checkValidity(resdata[i].target),
                                balance: this.checkValidity(resdata[i].balance),
                                current_club: this.checkValidity(resdata[i].currentClubBasisGWP.clubName),
                                next_club: this.checkValidity(resdata[i].nextClub.clubName),
                                _id_prod: this.checkValidity(resdata[i]._id),
                                // csm_emp_id: resdata[i].csm_emp_id ,
                                // csm_name: resdata[i].csm_name,
                        },
