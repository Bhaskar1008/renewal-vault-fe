...
<v-layout justify-start row>
	<!-- xs12 sm10 md8 lg6 -->
	<v-flex >
        <v-card ref="form" flat class="modifydd" light style="">
          <v-card-text>
            <v-autocomplete
              v-model="channelName"
...
<v-layout justify-end row>
        ...
        <v-card ref="form" flat class="modifydd2" light style="width:180px;">
          <v-card-text>
            			 <v-text-field 
		...
		<!-- <v-flex v-for="(listRejctData, index) in objRejct" :key="index"  class="pros-card-pass mt-2" xs12 lg6 > -->
				<v-flex  class="pros-card-pass mt-4" pl-2 pr-2 >
                        ...
                        <template slot="items" slot-scope="props" >                          
                            <td class="" @click="getProposalview(props.item.proposalid , props.index)">{{props.item.applicationcode}}</td>
                            <td class="" @click="getProposalview(props.item.proposalid , props.index)">{{props.item.name}}</td>
														<td class="" @click="getProposalview(props.item.proposalid , props.index)">{{props.item.status}}</td>
...
computed:{
       ...
       return this.objRejct.filter(function(cust){return cust.name.toLowerCase().indexOf(self.value.toLowerCase()) >=0
				|| cust.applicationcode.toLowerCase().indexOf(self.value.toLowerCase()) >=0
				 || cust.status.toLowerCase().indexOf(self.value.toLowerCase()) >=0
...
methods:{
	...
	getProposalData(chnl,skip){
			...
			if(chnl === 'all')
			{
				FilterData = 'getProposal?proposaltype=all&userId='+this.getAgentInfo()._id+'&skip='+skip
			}
			else
			{
				FilterData = 'getProposal?proposaltype=all&userId='+this.getAgentInfo()._id+'&channelCode='+chnl+'&skip='+skip
			}
			
			this.GET(FilterData,res => {
				   console.log('GET ALL DATA::',res.data.errMsg);
				   ...
				   for(let i = 0; i < rejctResp.length; i++) {
						...
						this.dataStruct.proposalId = rejctResp[i]._id;
						this.dataStruct.planName = rejctResp[i].productId.productName;
						console.log("PLAN NAME IS =>",rejctResp[i].productId.productName);
						this.dataStruct.createdDate = rejctResp[i].createdDate;
						this.dataStruct.applicationcode= rejctResp[i].application_code
						this.dataStruct.recieptingStatus = '-';
						this.dataStruct.specialCare = '-';
						this.dataStruct.status = rejctResp[i].proposalStatus == "Proposalstarted" ? "Draft" : rejctResp[i].proposalStatus == "issued" ? "Issued" : rejctResp[i].proposalStatus == "login" ? "Login" : rejctResp[i].proposalStatus == "rejected" ? "Rejected" : rejctResp[i].proposalStatus;
												//  
						this.dataStruct.proposalid = rejctResp[i]._id;
